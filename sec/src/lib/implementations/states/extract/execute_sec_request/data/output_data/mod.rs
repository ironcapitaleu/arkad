//! # `ExecuteSecRequestOutputData` Module
//!
...
//!
//! ## Types
//! - [`ExecuteSecRequestOutputData`]: Holds the SEC response after successful request execution.
//! - [`ExecuteSecRequestOutputDataUpdater`]: Updater type for modifying the output data in a controlled manner.
//! - [`ExecuteSecRequestOutputDataUpdaterBuilder`]: Builder for constructing updater instances with optional fields.
//!
//! ## Integration
...
//!
//! ## See Also
//! - [`super::input_data`]: Input data structure for request execution parameters.
...
impl ExecuteSecRequestOutputData {
    ...
    /// # Returns
    ///
    /// Returns a [`Result`] containing the new [`ExecuteSecRequestOutputData`] if successful.
    ///
    /// # Errors
    ///
...
/// Updater for modifying [`ExecuteSecRequestOutputData`] in a controlled manner.
///
...
/// Builder for constructing [`ExecuteSecRequestOutputDataUpdater`] instances.
///
...
impl ExecuteSecRequestOutputDataUpdaterBuilder {
    ...
    /// # Returns
    ///
    /// A new [`ExecuteSecRequestOutputDataUpdaterBuilder`] with all fields set to `None`.
    #[must_use]
    ...
    /// # Returns
    ///
    /// A new [`ExecuteSecRequestOutputDataUpdater`] with the fields set by this builder.
    #[must_use]
...
mod tests {
    ...
    fn should_create_new_output_data_with_provided_response() {
        ...
        let result = ExecuteSecRequestOutputData::new(response).expect("Should create output data");
    ...
    fn should_return_response_reference_when_accessing_response() {
        let response = SecResponse::default();
        let output_data =
            ExecuteSecRequestOutputData::new(response.clone()).expect("Should create output data");
    ...
    fn should_update_response_when_updater_contains_response() {
        ...
        let mut output_data =
            ExecuteSecRequestOutputData::new(original_response).expect("Should create output data");

        let updater = ExecuteSecRequestOutputDataUpdaterBuilder::new()
    ...
    fn should_not_update_fields_when_updater_is_empty() {
        ...
        let original_output_data =
            ExecuteSecRequestOutputData::new(response).expect("Should create output data");
        let mut output_data = original_output_data.clone();

        let updater = ExecuteSecRequestOutputDataUpdaterBuilder::new().build();
    ...
    const fn should_still_implement_auto_traits_when_implementing_state_data_trait() {
        implements_auto_traits::<ExecuteSecRequestOutputData>();
    }
    ...
    const fn should_implement_send_when_implementing_state_data_trait() {
        implements_send::<ExecuteSecRequestOutputData>();
    }

    #[test]
    const fn should_implement_sync_when_implementing_state_data_trait() {
        implements_sync::<ExecuteSecRequestOutputData>();
    }
