...
- Test function names are `snake_case`. `when` names a condition, `for` names a subject.
  ...
  - A compile-time trait witness holds unconditionally, so it always takes `for`.
...
- The knowledge is needed only for a specific subset of project work
- The content is substantial enough to waste context budget if always loaded
- The domain has procedures, invariants, or reference data that reduce hallucination
- The domain knowledge is expected to create meaningfully higher quality outputs

Do NOT create a skill when:

- The rule applies to all code universally → put it in AGENTS.md
- The content is a one-off decision → use memory
- You need to execute actions or query live systems → use a tool (MCP)
- The guidance is trivial or obvious from the code → neither
...
name: my-skill-name
description: >
  Use when [trigger conditions — what the user is working on or asking about].
version: 0.1.0
---
...
[One paragraph: what domain knowledge this provides and why it matters.]

## Procedures

[Step-by-step rules and decision procedures for the domain.]
...
- [Source name and URL or file path for live lookup]
```
