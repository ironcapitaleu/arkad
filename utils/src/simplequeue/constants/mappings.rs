...
mod tests {
    ...
    fn should_map_batch_extractor_connector_to_producer_access_for_batch_extractor_queue() {
        let configs = CONNECTOR_CONFIG_MAP.get(&ConnectorKind::BatchExtractor).expect("`ConnectorKind::BatchExtractor` should always have a config.");
    ...
    fn should_map_batch_transformer_connector_to_consumer_and_producer_access() {
        let configs = CONNECTOR_CONFIG_MAP.get(&ConnectorKind::BatchTransformer).expect("`ConnectorKind::BatchTransformer` should always have a config.");
    ...
    fn should_map_batch_loader_connector_to_consumer_access_for_batch_transformer_queue() {
        let configs = CONNECTOR_CONFIG_MAP.get(&ConnectorKind::BatchLoader).expect("`ConnectorKind::BatchLoader` should always have a config.");
