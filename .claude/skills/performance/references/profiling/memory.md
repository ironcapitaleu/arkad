...
## Known blocker: memory under concurrency

Measuring a full multi-CIK run without hitting the live SEC API needs a client-injection seam that does
not exist: the state contexts hold the **concrete** `SecClient` struct rather than a generic
`C: SecClient`, so no fake can be threaded through. The existing fakes are also placeholders
